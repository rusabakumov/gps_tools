from gpstools.gps_utils import calculate_point_stats, calculate_dist_travelled
...
class ReferenceTrack:
    ...
    def __init__(self, track):
        ...
        assert len(self.reference_points) > 0

        self.len = len(self.reference_points)
        self.lat, self.lon, self._time, self._dist, self._micros = calculate_point_stats(self.reference_points)
        self.dist_travelled = calculate_dist_travelled(self._dist)

        self.start_point = self.reference_points[0]
        self.finish_point = self.reference_points[-1]
